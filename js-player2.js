...
class jsplayer extends HTMLElement{
    ...
    $video_timeupdate(event){
        ...
        this.bufferRender()
        this.$timeCurrent.textContent = this.timeFormat(current)
        this.$timeTotal.textContent   = this.timeFormat(duration)
        this.thumbRender(this.$seekThumb, current/duration)
    ...
    $seekArea_mousedown(event){
        this.timeSet(this.$video.duration * this.percentageX(this.$seekArea, event.clientX))
    }
    ...
    $volumeArea_mousedown(event){
        this.$video.muted = false
        this.volumeSet(this.percentageX(this.$volumeArea, event.clientX))
    }
    ...
    percentageX($area, currentX){
        const {x, width} = $area.getBoundingClientRect()
    ...
    commentPost(){
        ...
        if(text && this.post && this.$video.readyState){
            ...
            this.comments[Math.floor(time+1)]?.unshift([text, time+1])
